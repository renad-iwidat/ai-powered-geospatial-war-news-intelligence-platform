#!/usr/bin/env python3
"""
Script to fix actual locations in database
تصحيح المواقع الفعلية في قاعدة البيانات
"""
...
# ============================================================================
# قائمة التصحيحات الفعلية
# ============================================================================
COORDINATES_TO_FIX = {
    # (name, country_code): (new_lat, new_lng, reason)
    ('غزه،', 'PS'): (31.5017, 34.4668, 'تصحيح الإحداثيات - كانت قرب الضفة'),
    ('عمان', 'OM'): (31.9539, 35.9106, 'تصحيح: هذا يجب أن يكون عمّان الأردن وليس عمّان عمان'),
}


async def fix_locations():
    """تصحيح المواقع الفعلية"""
    ...
    try:
        print("=" * 70)
        print("تصحيح المواقع الفعلية في قاعدة البيانات")
        print("=" * 70)
        print()
        ...
        async with pool.acquire() as conn:
            ...
            for (name, country_code), (new_lat, new_lng, reason) in COORDINATES_TO_FIX.items():
                row = await conn.fetchrow(
                    "SELECT id, latitude, longitude FROM locations WHERE name = $1 AND country_code = $2",
                    name,
                    country_code
                )
